...
class Solution {
    boolean isSumTree(Node root) {
        ...
        int sum = 0;
        if(root.left == null) sum = findSum(root.right);
        else if(root.right == null) sum = findSum(root.left);
        else{
    ...
    int findSum(Node node)
    {
        ...
        int sum = 0;
        if(node.left == null) sum = findSum(node.right);
        else if(node.right == null) sum = findSum(node.left);
        else{
